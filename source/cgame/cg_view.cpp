...
static int CG_CalcFov( void ) {
	...
	if(!cg.renderingThirdPerson && (cg_trueguns.integer || cg.predictedPlayerState.weapon == WP_SABER
		|| cg.predictedPlayerState.weapon == WP_MELEE || cg.predictedPlayerState.weapon == WP_TUSKEN_RIFLE) 
...
void CG_DrawSkyBoxPortal(const char *cstr)
{
	...
	if (!fov_x)
	{
		...
		|| cg.predictedPlayerState.weapon == WP_TUSKEN_RIFLE) && cg_truefov.value
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	{		
		...
		CGCam_UpdateFade();
		// build cg.refdef
		inwater = (qboolean)CG_CalcViewValues();
	}
	// build cg.refdef
