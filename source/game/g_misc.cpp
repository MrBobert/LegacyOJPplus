...
/*char *holocronTypeModels[] = {
	...
	"models/chunks/rock/rock_big.md3",//FP_MINDTRICK,
...
qboolean HasValidWeaponThatUsesAmmo( gentity_t *ent, int ammotype ) {
	switch ( ammotype ) {
		...
		case AMMO_TUSKEN_RIFLE:
			if( (ent->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_TUSKEN_RIFLE) ) )
				return qtrue;
			break;
		...
		case AMMO_THERMAL:
			...
			break;
		case AMMO_TRIPMINE:
			if ( (ent->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_GRENADE ) ) || 
